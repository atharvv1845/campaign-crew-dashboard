...
const LeadTableRow: React.FC<LeadTableRowProps> = ({
  ...
  const [editNote, setEditNote] = useState<string>(lead.notes || '');
  ...
  const updateLeadStage = (stage: string) => {
    if (onUpdateLead) {
  ...
  const updateLeadAssignment = (teamMember: string) => {
    if (onUpdateLead) {
  ...
  const updateFollowUpDate = (date: Date) => {
    if (onUpdateLead) {
      const updatedLead = { 
        ...lead, 
    ...
    <tr className="hover:bg-muted/10 transition-colors">
      ...
      <td className="px-6 py-4">
        <FollowUpDateSelector 
          ...
          onDateSelected={(date) => updateFollowUpDate(date)} 
        />
      </td>
      <td className="px-6 py-4">
        <StageSelector 
      ...
      <td className="px-6 py-4">
        <TeamMemberSelector 
          ...
          onTeamMemberChange={updateLeadAssignment} 
        />
      </td>
      ...
      <td className="px-3 py-4">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={logContact}>
              Log Contact
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
...
const SocialLinks: React.FC<{ lead: Lead }> = ({ lead }) => {
  return (
...
// FollowUpDateSelector Component
const FollowUpDateSelector: React.FC<{ 
  followUpDate?: string; 
  onDateSelected: (date: Date) => void 
}> = ({ followUpDate, onDateSelected }) => {
  return (
    <Popover>
...
const TeamMemberSelector: React.FC<{ 
  assignedTo: string; 
  teamMembers?: string[];
  onTeamMemberChange: (member: string) => void;
}> = ({ assignedTo, teamMembers, onTeamMemberChange }) => {
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-7">
